...
export const ImageAdjuster: React.FC<ImageAdjusterProps> = ({ onCancel, onRotate90, onConfirm }) => {
  ...
  return (
    <div className="absolute inset-0 z-30 pointer-events-none">
      ...
      <div className="absolute bottom-4 left-4 w-16 h-16 border-b-4 border-l-4 border-gray-800/70 rounded-bl-md"></div>
      <div className="absolute bottom-4 right-4 w-16 h-16 border-b-4 border-r-4 border-gray-800/70 rounded-br-md"></div>

      {showPinchHint && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-50">
          <div className="bg-black/60 p-4 rounded-lg animate-pinch-hint">
            <img src={pinchIconBase64} alt="ピンチ操作で調整" className="w-20 h-20 invert" />
          </div>
        </div>
      )}
