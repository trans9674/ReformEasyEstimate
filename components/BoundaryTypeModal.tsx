    ...
    <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={onClose}>
      <div className="bg-white p-6 rounded-lg shadow-xl text-center max-w-sm w-full" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex flex-col gap-4">
          <button onClick={() => onSelect('隣地境界線')} className="w-full px-6 py-3 bg-blue-500 text-white rounded-md hover:bg-blue-600 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-400 font-semibold">
            隣地境界線
          </button>
          <button onClick={() => onSelect('道路境界線')} className="w-full px-6 py-3 bg-green-500 text-white rounded-md hover:bg-green-600 transition-colors focus:outline-none focus:ring-2 focus:ring-green-400 font-semibold">
            道路境界線
          </button>
